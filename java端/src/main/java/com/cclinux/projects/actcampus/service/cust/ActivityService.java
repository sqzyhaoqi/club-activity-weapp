...
public class ActivityService extends BaseMyCustService {
    ...
    public long activityJoin(long userId, long activityId, String forms, String obj) {
        ...
        ActivityModel activityModel = this.activityMapper.getOne(activityId);
        long activityMaxCnt = activityModel.getActivityMaxCnt();
        long activityJoinCnt = activityModel.getActivityJoinCnt();
        if (activityJoinCnt == activityMaxCnt) {
            String activityTitle = activityModel.getActivityTitle();
            appError("{" + activityTitle + "}该活动已超出最大人员限定！");
        }
        activityModel.setActivityJoinCnt(activityJoinCnt + 1);
        this.activityMapper.updateById(activityModel);

        String tel = JSONUtil.parseObj(obj).getStr("phone");

        Where<ActivityJoinModel> where = new Where<>();
        where.eq("ACTIVITY_JOIN_CODE", tel);
        ActivityJoinModel activityJoin = activityJoinMapper.getOne(where);
        if (!ObjectUtil.isEmpty(activityJoin)) throw new AppException("该用户已经报名，请勿重复报名！");

        ActivityJoinModel activityJoinModel = new ActivityJoinModel();
        activityJoinModel.setActivityJoinActivityId(activityId);
        activityJoinModel.setActivityJoinUserId(userId);
        activityJoinModel.setActivityJoinForms(forms);
        activityJoinModel.setActivityJoinObj(obj);
        activityJoinModel.setActivityJoinCode(tel);
        return this.activityJoinMapper.insert(activityJoinModel);
    }
